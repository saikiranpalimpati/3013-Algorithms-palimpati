...
public:
	...
	*/
	...
	void createSpanningTree(string filter = "")
	{
		...
		cout << "Creating Spanning Tree...Wait sometime for processing.....\n";
		for (int i = 0; i<vertexList.size(); i++)
		{
			...
			for (int j = i + 1; j < vertexList.size(); j++)
			{
				...
				double distance = distanceEarth(vertexList[i]->loc.lat, vertexList[i]->loc.lon, vertexList[j]->loc.lat, vertexList[j]->loc.lon);
				//cout << distance<<endl;
				if (distance >= 0)
				{
					addEdge(vertexList[i]->ID, vertexList[j]->ID, distance, false);
					cnt++;
					n1 = distance;
					//cout << distance << endl;
					if (cnt > maxdegree)
		...
		cout << "Spanning Tree Creation Successful\n";
		...
		for (vit = vertexList.begin(); vit != vertexList.end(); vit++)
		{
			if ((*vit)->E.size() > 0 && ((*vit)->state == "PR"))
			{
				...
				for (eit = (*vit)->E.begin(); eit != (*vit)->E.end(); eit++)
				{
					...
					if ((vertexList[eit->toID]->state == "PR"))
					{
						if (eit->toID <= 40)
						...
						int id = ((*vit)->ID) % 40;
						int id2 = (eit->toID) % 40;
						conns += "\t" + to_string(id) + arrow
							+ to_string(id2) + " [weight=" + to_string(weight) + ", label=" + to_string(weight) + "]\n";
						cnt++;
